...
import com.antontkatch.restaurant.util.VoteUtil;
...
import static com.antontkatch.restaurant.util.VoteUtil.checkTime;
...
public class VoteController {
    ...
    public ResponseEntity<Vote> createOrUpdate(@RequestBody VoteTo voteTo, @PathVariable int restaurantId, @AuthenticationPrincipal AuthUser authUser) {
        int userId = authUser.getUser().id();
        Optional<Vote> voteDb = service.getTodayVoteIfExists(userId);
        Vote newVote;
        if (voteDb.isPresent() && checkTime(voteTo)) {
            return ResponseEntity.ok(voteDb.get());
        } else newVote = voteDb.orElseGet(() -> VoteUtil.createNewFromTo(voteTo));
